...
function testReadmeGenerator() {
  ...
  try {
    ...
    const args = [generatorPath, configPath, readmePath, templatePath];
    if (debug) args.push('--debug');
    if (debug) {
      console.log('[test.js] Debug: Generator args:', args);
    }
    execFileSync('node', args, { cwd: process.cwd(), stdio: debug ? 'inherit' : 'pipe' });
